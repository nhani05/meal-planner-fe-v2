...
export default function Templates() {
  ...
  const handleCreate = async () => {
    if (!newName.trim()) {
      setCreateError(t('toast.templateNameRequired'));
      return;
    }
    if (!newSourcePlanId) {
      setCreateError(t('templatesPage.selectSourcePlan'));
      return;
    }
    setCreateError('');
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-[#171d16]">{t('templatesPage.title')}</h2>
          <p className="text-sm text-[#6f7a6b] font-medium">
            {t('templatesPage.savedCount', { count: templates.length })}
          </p>
        </div>
        <button
      ...
      {isLoadingTemplates ? (
        ...
        <div className="bg-white rounded-2xl border border-[#becab9] shadow-card p-12 text-center">
          ...
          <h3 className="font-bold text-[#171d16] mb-1">{t('templatesPage.noTemplates')}</h3>
          <p className="text-sm text-[#6f7a6b] mb-6">{t('templatesPage.noTemplatesText')}</p>
          <button
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      {isEditing ? (
                        ...
                        <h3 className="font-bold text-[#171d16] truncate">{template.templateName || template.name || t('templatesPage.untitled')}</h3>
                      )}
                      <div className="flex items-center gap-1.5 text-[10px] text-[#6f7a6b] mt-1">
                        <Calendar size={12} />
                        <span>
                          {template.savedAt || template.createdAt
                            ? new Date(template.savedAt || template.createdAt).toLocaleDateString(i18n.resolvedLanguage === 'en' ? 'en-US' : 'vi-VN', { month: 'short', day: 'numeric', year: 'numeric' })
                            : t('templatesPage.unknownDate')}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-[#eaf0e4]">
                <div>
                  <h2 className="font-bold text-[#171d16]">{t('templatesPage.newTemplate')}</h2>
                  <p className="text-[10px] font-bold text-[#6f7a6b] uppercase tracking-wider mt-0.5">{t('templatesPage.savePlanAsTemplate')}</p>
                </div>
                <button
                  onClick={() => setIsCreateOpen(false)}
              ...
              <div className="p-6 space-y-4">
                <div>
                  <label className="text-xs font-bold text-[#6f7a6b] uppercase tracking-wider mb-1.5 block">{t('templatesPage.templateName')}</label>
                  <input
                    type="text"
                    placeholder={t('templatesPage.templatePlaceholder')}
                    value={newName}
                ...
                <div>
                  <label className="text-xs font-bold text-[#6f7a6b] uppercase tracking-wider mb-1.5 block">{t('templatesPage.sourcePlan')}</label>
                  <select
                    value={newSourcePlanId}
                    onChange={(e) => { setNewSourcePlanId(e.target.value); setCreateError(''); }}
                    className="w-full px-4 py-3 rounded-xl bg-[#f0f6ea] border-transparent focus:bg-white focus:border-[#4caf50] focus:ring-2 focus:ring-[#4caf50]/20 transition-all text-sm outline-none"
                  >
                    <option value="">{t('templatesPage.selectPlan')}</option>
                    {plans.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.planName || `${t('templatesPage.plan')} ${p.planDate}`} ({p.planDate})
                      </option>
                    ))}
                  </select>
                  {plans.length === 0 && (
                    <p className="text-xs text-[#ba1a1a] mt-1.5">{t('templatesPage.noPlans')}</p>
                  )}
                </div>
